function DtaPntBox(DataBoxes, Labels, LabelsScatter, ClrSheme, ClrShemeSct, Alp, XTickName, yAxisName, Wdth, MkrSiz, RepMean, Lgd, Textex, Save)
%
...
%                     vectors with colors as many unique labels in
%                     LabelsScatter. Leave empty if data points should be
%                     black.
% 6) Alp            - Alpha value(range from 0 to 1) for boxes. Defoult = 0.25
%
% 7) XTickName      - Names for different columns from DataBoxes, e.g.: Treatment A,
%                     treatment B, (...) treatment F
%
% 8) yAxisName      - Label for Y axis
%
% 9) Wdth           - Line width for line plot
%
% 10) MkrSiz         - Marker size for scatter plot. Empty if without markers
%
% 11) RepMean       - Input 'Yes' or 1 for mean group representation
%
% 12) Lgd           - Input 'Yes' or 1 for displaying legend
%
% 13) Textex        - Title of boxplot and name of file if saved
%
% 14) Save          - Input 'Yes' or 1, if you want save graphical output into *.TIF file
%
...
f = [1 2 3 4];
patch('Faces',f,'Vertices',a1,'FaceColor',ClrSheme(i,:),'FaceAlpha',Alp);
end
...
for i=1:length(x)
   ...
   CmbFig(i) = scatter(NaN,NaN,10,'s','LineWidth',10,'MarkerFaceColor',ClrSheme(i,:),'MarkerEdgeColor',ClrSheme(i,:),'MarkerFaceAlpha',Alp,'MarkerEdgeAlpha',Alp,'DisplayName', xIn);
end
